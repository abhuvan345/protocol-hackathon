...
const Judges = () => {
  ...
  useEffect(() => {
    if (selectedJudge) { fetchTopTeams(); fetchExistingScores(); }
  }, [selectedJudge, config.problemStatements]);

  const fetchTopTeams = async () => {
    setLoading(true);
    const { data: regs } = await supabase.from("registrations").select("*");
    const { data: mentorScoresData } = await supabase.from("mentor_scores").select("*");
    if (!regs || !mentorScoresData) { setLoading(false); return; }
  ...
  const fetchExistingScores = async () => {
    ...
    const { data } = await supabase.from("judge_scores").select("team_id").eq("judge_name", selectedJudge);
    if (data) {
  ...
  const openScoring = (team: any) => { setScoringTeam(team); setScores({}); };

  const handleSubmitScore = async () => {
    if (!selectedJudge || !scoringTeam) return;
    const allFilled = rubricCategories.every((c) => scores[c] >= 1 && scores[c] <= 5);
    if (!allFilled) { toast.error("Please select a score (1-5) for all categories"); return; }
      ...
      <div className="min-h-screen bg-background">
        <header className="border-b border-border bg-card">
          <div className="container mx-auto px-4 py-4 flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">Judge Scoring</h1>
          </div>
        </header>
        <main className="container mx-auto px-4 py-10 max-w-md space-y-6">
          ...
          <div className="grid gap-3">
            {config.judges.map((j) => (
              <Card key={j} className="p-4 cursor-pointer hover:border-primary/30 hover:shadow-md transition-all border-2 border-transparent" onClick={() => setSelectedJudge(j)}>
      ...
      <div className="min-h-screen bg-background">
        <header className="border-b border-border bg-card">
          <div className="container mx-auto px-4 py-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Judge: {selectedJudge} | PS {scoringTeam.problem_statement}</p>
            </div>
          </div>
        </header>
        <main className="container mx-auto px-4 py-6 max-w-4xl space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="text-lg font-bold">Total: <span className="text-primary">{totalRubric}</span>/25</div>
      ...
      <header className="border-b border-border bg-card">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={() => setSelectedJudge(null)}><ArrowLeft className="h-4 w-4" /></Button>
            <h1 className="text-xl font-bold">Judge: {selectedJudge}</h1>
          </div>
        </div>
      </header>
      <main className="container mx-auto px-4 py-8 max-w-3xl space-y-8">
        {loading ? (
          <p className="text-center text-muted-foreground">Loading top teams...</p>
        ) : (
          config.problemStatements.map((ps) => (
            <div key={ps.id}>
              ...
              {(topTeams[ps.id] || []).length === 0 ? (
                <p className="text-sm text-muted-foreground ml-7">No scored teams yet for this PS.</p>
              ) : (
                    ...
                    <Card key={team.id} className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-muted-foreground ml-2">(Mentor avg: {team.mentorAvg.toFixed(1)}/25)</span>
                        </div>
                      </div>
