...
const Mentors = () => {
  ...
  const fetchTeams = async () => {
    ...
    const { data } = await supabase
      .from("registrations")
      .select("*")
      .eq("problem_statement", selectedMentor.ps)
  ...
  const handleSubmitScore = async () => {
    ...
    const allFilled =
      rubricCategories.every((c) => scores[c] >= 1 && scores[c] <= 5) &&
      pptScore >= 1 &&
      pptScore <= 5;
    if (!allFilled) {
      toast.error(
        "Please select a score (1-5) for all categories including PPT",
      );
      return;
    }
      ...
      <div className="min-h-screen bg-background">
        <header className="border-b border-border bg-card">
          <div className="container mx-auto px-4 py-4 flex items-center gap-2 sm:gap-3">
            ...
            <h1 className="text-lg sm:text-xl font-bold">Mentor Scoring</h1>
          </div>
        </header>
          ...
          <div className="container mx-auto px-4 py-4 flex items-start gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">
                Mentor: {selectedMentor.name} | PS {selectedMentor.ps}
              </p>
            </div>
          </div>
        </header>
          ...
          <Card className="p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {[1, 2, 3, 4, 5].map((s) => (
                <Button
                  key={s}
                  variant={pptScore === s ? "default" : "outline"}
                  size="sm"
                  onClick={() => setPptScore(s)}
                >
                  {s}
                </Button>
              ))}
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-base sm:text-lg font-bold">
              ...
              <span className="block sm:inline text-sm text-muted-foreground sm:ml-2">
                (Rubric: {totalRubric}/25 + PPT: {pptScore}/5)
              </span>
            </div>
            <Button
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                PS {selectedMentor.ps} — {psNames[selectedMentor.ps]}
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="container mx-auto px-4 py-8 max-w-2xl space-y-4">
        ...
        {teams.length === 0 ? (
          <p className="text-muted-foreground">
            No teams registered for this problem statement yet.
          </p>
        ) : (
          teams.map((team) => (
            <Card
            ...
            >
              <span className="font-medium break-words">{team.team_name}</span>
